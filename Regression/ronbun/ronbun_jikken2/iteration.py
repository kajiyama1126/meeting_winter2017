...
class Iteration_multi(object):
    ...
    def optimal(self):  # L1
        ...
        self.A = 0.4 * np.array([np.random.randn(self.m, self.m) for i in range(self.n)])
    ...
    def iteration(self, pattern, stop_condition):
        ...
        for k in range(self.iterate):
            ...
            ##if error < stop_condition or k==self.iterate:
            #    return k
...
class Iteration_multi_graph(Iteration_multi):
    def __init__(self, n, m, eta, pattern, count):
        ...
        #self, n, m, parameter, algo, count, stop_condition
    ...
    def main(self):
        ...
        for test in range(self.count):
            ...
            for i in range(self.pattern):
                 #cont, graph, ydata_zdata = self.iteration(i, stop_condition)
                 cont, graph, ydata_zdata = self.iteration(i, stop_condition)
        ...
        #label = ['[20]', 'Proposed']
        #label = ['Proposed', '[20]', '[34]', '[35]', '[36]']
        label = ['Proposed', '[34]', '[35]', '[36]']
        #label = ['Harnessing', 'Proposed', 'Subgrad', 'ADMM']
        #line = ['-', '--', '-.', ':', '.']
        #line = ['-', '--', '-.', ':']
        line = ['-', '-', '-', '-']
        #line = ['-', '-', '-', '-', '-']
        ...
        for i in range(self.pattern):
            plt.plot(iterate_graph[i], label=label[i], linestyle=line[i], linewidth=2)
            # plt.legend()
            # plt.yscale('log')
        ...
        plt.tight_layout()
        plt.savefig("cost_compare.png")
        plt.savefig("cost_compare.eps")
        plt.show()
    ...
    def iteration(self, pattern, stop_condition):
        ...
        for k in range(self.iterate):
            ...
            error = np.max(f_value) - self.f_opt
            # if error < stop_condition:
            #     #if error < stop_condition or k==self.iteration:
            #     #if error < stop_condition
